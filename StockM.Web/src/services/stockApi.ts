...
const UPSTOX_URL = 'https://api.upstox.com';
...
// Common NSE/BSE stock symbol → ISIN mapping for instant instrument key resolution
const NSE_ISIN_MAP: Record<string, string> = {
  RELIANCE: 'INE002A01018', TCS: 'INE467B01029', HDFCBANK: 'INE040A01034',
  INFY: 'INE009A01021', ICICIBANK: 'INE090A01021', BHARTIARTL: 'INE397D01024',
  SBIN: 'INE062A01020', ITC: 'INE154A01025', HINDUNILVR: 'INE030A01027',
  LT: 'INE018A01030', BAJFINANCE: 'INE296A01024', TITAN: 'INE280A01028',
  WIPRO: 'INE075A01022', HCLTECH: 'INE860A01027', MARUTI: 'INE585B01010',
  SUNPHARMA: 'INE044A01036', ASIANPAINT: 'INE021A01026', AXISBANK: 'INE238A01034',
  KOTAKBANK: 'INE237A01028', ADANIENT: 'INE423A01024', TATAMOTORS: 'INE155A01022',
  TATASTEEL: 'INE081A01020', NTPC: 'INE733E01010', POWERGRID: 'INE752E01010',
  ONGC: 'INE213A01029', COALINDIA: 'INE522F01014', JSWSTEEL: 'INE019A01038',
  NESTLEIND: 'INE239A01016', ULTRACEMCO: 'INE481G01011', BAJAJFINSV: 'INE918I01018',
  TECHM: 'INE669C01036', INDUSINDBK: 'INE095A01012', DRREDDY: 'INE089A01023',
  DIVISLAB: 'INE361B01024', CIPLA: 'INE059A01026', EICHERMOT: 'INE066A01021',
  APOLLOHOSP: 'INE437A01024', GRASIM: 'INE047A01021', HEROMOTOCO: 'INE158A01026',
  BPCL: 'INE029A01011', TATACONSUM: 'INE192A01025', M_M: 'INE101A01026',
  HINDALCO: 'INE038A01020', BRITANNIA: 'INE216A01030', NIFTY: 'Nifty 50',
};
...
// ===================== UPSTOX API (NSE/BSE — REAL HISTORICAL DATA) =====================

// Instrument key cache to avoid repeated searches
const instrumentKeyCache = new Map<string, string>();

function getUpstoxHeaders(token: string): Record<string, string> {
  return {
...
/** Resolve a trading symbol to an Upstox instrument key (e.g. NSE_EQ|INE002A01018) */
export async function resolveUpstoxInstrumentKey(
  symbol: string, token: string, market: Market = 'NSE'
): Promise<string> {
  const exchange = market === 'BSE' ? 'BSE' : 'NSE';
  const cacheKey = `${exchange}:${symbol}`;

  if (instrumentKeyCache.has(cacheKey)) {
    return instrumentKeyCache.get(cacheKey)!;
  }

  // Check hardcoded ISIN map first
  // Handle M&M special case
  const lookupSymbol = symbol === 'M&M' ? 'M_M' : symbol;
  const isin = NSE_ISIN_MAP[lookupSymbol];
  if (isin) {
    const key = `${exchange}_EQ|${isin}`;
    instrumentKeyCache.set(cacheKey, key);
    return key;
  }

  // Fallback: use Instrument Search API
  const url = `${UPSTOX_URL}/v2/instruments/search?query=${encodeURIComponent(symbol)}&exchanges=${exchange}&segments=EQ&records=5`;
  const res = await fetch(url, { headers: getUpstoxHeaders(token) });
  if (!res.ok) throw new Error(`Upstox instrument search returned ${res.status}`);

  const json = await res.json();
  if (json.status !== 'success' || !json.data?.length) {
    throw new Error(`Upstox: no instrument found for ${symbol}`);
  }

  // Find exact trading symbol match, or first EQ result
  const match = json.data.find((d: any) => d.trading_symbol === symbol && d.segment === `${exchange}_EQ`)
    || json.data[0];
  const key = match.instrument_key;
  instrumentKeyCache.set(cacheKey, key);
  return key;
}

/** Fetch real historical daily OHLC candles from Upstox (up to 10 years!) */
export async function fetchUpstoxHistorical(
  symbol: string, token: string, market: Market = 'NSE', days = 1300
): Promise<StockBar> {
  const instrumentKey = await resolveUpstoxInstrumentKey(symbol, token, market);
  const encodedKey = encodeURIComponent(instrumentKey);

  const toDate = new Date().toISOString().split('T')[0];
  const fromDate = new Date(Date.now() - days * 24 * 60 * 60 * 1000).toISOString().split('T')[0];

  const url = `${UPSTOX_URL}/v3/historical-candle/${encodedKey}/days/1/${toDate}/${fromDate}`;
  const res = await fetch(url, { headers: getUpstoxHeaders(token) });
  if (!res.ok) throw new Error(`Upstox historical returned ${res.status}`);

  const json = await res.json();
  if (json.status !== 'success' || !json.data?.candles?.length) {
    throw new Error(`Upstox: no historical data for ${symbol}`);
  }

  // Candle format: [timestamp, open, high, low, close, volume, oi]
  const quotes: StockQuote[] = json.data.candles
    .map((c: any[]) => ({
      timestamp: c[0].split('T')[0], // "2025-01-01T00:00:00+05:30" → "2025-01-01"
      open: c[1],
      high: c[2],
      low: c[3],
      close: c[4],
    ...
    .sort((a: StockQuote, b: StockQuote) => a.timestamp.localeCompare(b.timestamp));

  return { symbol, quotes };
}

/** Fetch full market quote from Upstox (live OHLC, last_price, volume) */
export async function fetchUpstoxQuote(
  symbol: string, token: string, market: Market = 'NSE'
): Promise<LiveQuote> {
  const instrumentKey = await resolveUpstoxInstrumentKey(symbol, token, market);
  const encodedKey = encodeURIComponent(instrumentKey);

  const url = `${UPSTOX_URL}/v2/market-quote/quotes?instrument_key=${encodedKey}`;
  const res = await fetch(url, { headers: getUpstoxHeaders(token) });
  if (!res.ok) throw new Error(`Upstox quote returned ${res.status}`);

  const json = await res.json();
  if (json.status !== 'success' || !json.data) {
    throw new Error(`Upstox: no quote data for ${symbol}`);
  }

  // The data key uses colon format: "NSE_EQ:RELIANCE"
  const dataKey = Object.keys(json.data)[0];
  const d = json.data[dataKey];
  if (!d) throw new Error(`Upstox: empty quote for ${symbol}`);

  return {
    symbol,
    companyName: d.symbol || symbol,
    lastPrice: d.last_price,
    change: d.net_change ?? 0,
    percentChange: d.last_price && d.ohlc?.close ? round(((d.last_price - d.ohlc.close) / d.ohlc.close) * 100) : 0,
    previousClose: d.ohlc?.close ?? 0,
    open: d.ohlc?.open ?? 0,
    dayHigh: d.ohlc?.high ?? 0,
    dayLow: d.ohlc?.low ?? 0,
    yearHigh: d.upper_circuit_limit ?? 0,
    yearLow: d.lower_circuit_limit ?? 0,
    volume: d.volume ?? 0,
    marketCap: 0,
    peRatio: 0,
    dividendYield: 0,
...
/** Fetch multiple Upstox quotes in a single batched call (up to 500 instrument keys) */
export async function fetchUpstoxMultipleQuotes(
  symbols: string[], token: string, market: Market = 'NSE'
): Promise<LiveQuote[]> {
  // Resolve all instrument keys
  const keys: string[] = [];
  for (const s of symbols.slice(0, 20)) {
    try {
      keys.push(await resolveUpstoxInstrumentKey(s, token, market));
    } catch { /* skip unresolvable */ }
  }
  if (keys.length === 0) return [];

  const url = `${UPSTOX_URL}/v2/market-quote/quotes?instrument_key=${encodeURIComponent(keys.join(','))}`;
  const res = await fetch(url, { headers: getUpstoxHeaders(token) });
  if (!res.ok) return [];
  ...
  for (const dataKey of Object.keys(json.data)) {
    const d = json.data[dataKey];
    if (!d) continue;
    const sym = d.symbol || dataKey.split(':')[1] || dataKey;
    results.push({
      symbol: sym,
      companyName: sym,
      lastPrice: d.last_price ?? 0,
      change: d.net_change ?? 0,
      percentChange: d.last_price && d.ohlc?.close ? round(((d.last_price - d.ohlc.close) / d.ohlc.close) * 100) : 0,
      previousClose: d.ohlc?.close ?? 0,
      open: d.ohlc?.open ?? 0,
